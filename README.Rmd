...
[![CRAN status](https://www.r-pkg.org/badges/version/CohortSymmetry)](https://CRAN.R-project.org/package=CohortSymmetry)
[![Codecov test coverage](https://codecov.io/gh/OHDSI/CohortSymmetry/graph/badge.svg)](https://app.codecov.io/gh/OHDSI/CohortSymmetry)
...
If one wants to generate two drugs cohorts in cdm, the [DrugUtilisation R package](https://darwin-eu.github.io/DrugUtilisation/) is recommended. For merely illustration purposes, we will carry out SSA on aspirin (index_cohort) against amoxicillin (marker_cohort). Multiple markers can be instantiated in the marker cohort and each one will be tested against the index cohort.
...
Note that gt is also an option, users may specify this by using the `type` argument.
